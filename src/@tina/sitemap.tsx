...
async function* getAllPages() {
  ...
  while (hasNextPage) {
    ...
    yield* response.data.pageConnection.edges!.map((edge: any) => edge.node);
...
async function* getAllPosts() {
  ...
  while (hasNextPage) {
    ...
    yield* response.data.postConnection.edges!.map((edge: any) => edge.node);
...
async function* getAllFeedItems() {
  ...
  const posts = response.data.postConnection.edges.map(({ node }: any) => node);
  for (const post of posts) {
    ...
    switch (post.__typename) {
      case 'PostAudio':
        // @ts-expect-error TS(2339) FIXME: Property 'audio' does not exist on type '{ title: ... Remove this comment to see the full error message
        item.audio = {
          url: post.audio.url,
        };
        break;
 ...
 * @returns
 */
const postToSitemapItem = (post: any) => {
  /** @type {SitemapItem} */
  const item = {
    img: [],
    video: [],
 ...
 * @returns {SitemapItem}
 */
const pageToSitemapItem = (post: any) => {
  /** @type {SitemapItem} */
  const item = {
    img: [],
    video: [],
...
const pageToSitemapMapper = {
  ...
  async *[FIVE_HUNDRED_SLUG](page: any) {
    return;
  },
  async *[FOUR_OH_FOUR_SLUG](page: any) {
    return;
  },

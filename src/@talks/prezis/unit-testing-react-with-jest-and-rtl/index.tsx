...
const code = {
  enzyme: `const wrapper = mount(<SomeComponent />)

wrapper.instance().someClassMethod()

expect(wrapper.state()).toEqual({ calledClassMethod: true })`,
  rtl: `const { container, getByText } = render(<SomeComponent />)
  ...
  <Deck transitionDuration={500} theme={DarkTheme} transition={['slide']}>
    ...
    <Slide>
      ...
      <List>
        ...
        <Appear>
          <ListItem>More confidence in solution</ListItem>
        </Appear>
        <Appear>
          <ListItem>Documentation of developer intentions</ListItem>
        </Appear>
        <Appear>
    ...
    <Slide>
      ...
      <Text>
        RTL (react-testing-library) tests components similar to how your users
        interact with them.
      </Text>
    </Slide>
    <Slide>
    ...
    <Slide>
      ...
      <CodePane
        textSize={'2em'}
        source={'npx create-react-app tdd-with-jest'}
      />
    </Slide>
    <Slide>
      <Heading size={3} margin={'0 0 30px'}>
      ...
      <List ordered>
        ...
        <ListItem>
          Should not submit unless password is greater than 8 characters
        </ListItem>
        <ListItem>

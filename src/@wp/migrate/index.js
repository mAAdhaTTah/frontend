...
export const contentToChildren = content => {
  ...
  let textNode = null;
  const lastElement = () => {
    const currentChil = currentChildren();
    return currentChil[currentChil.length - 1];
  };
  const lastIsCodeBlock = () => {
    return lastElement()?.type === 'code_block';
  };
    ...
    onopentag(name, attribs, isImplied) {
      ...
      switch (name) {
        ...
        case 'span':
          break;
        case 'pre':
          const lang = attribs.class?.replace('language-', '');
          const pre = {
            type: 'code_block',
            lang,
            value: '',
          ...
          currentChildren().push(pre);
          stack.push(pre.children);
          break;
        case 'br':
          currentChildren().push({ type: 'break', text: '' });
          break;
        case 'p':
        case 'h2':
        case 'h3':
...
${content}`);
    ...
    ontext(text) {
      ...
      if (textNode) {
        textNode.text += text;
      } else if (lastIsCodeBlock()) {
        lastElement().value = text;
      } else {
    ...
    onclosetag(name, isImplied) {
      ...
      if (!['em', 'strong', 'code', 'br'].includes(name) || lastIsCodeBlock()) {

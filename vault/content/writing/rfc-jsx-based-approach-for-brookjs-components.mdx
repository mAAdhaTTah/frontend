...
!\[\[Brookjs components strawman proposal#^TodoItem-js]]
...
!\[\[Brookjs components strawman proposal#^App-js]]

Even attributes on a component **must** be expressed as an Observable. Again, *anything dynamic **must** be expressed as an Observable*. This allows the developer to optimize rendering at a very granular level. Individual attributes can be `filter`ed, and `skipDuplicates` becomes a version of React's `shouldComponentUpdate`, but instead of needing a single function to account for the entire component, we can write a function for each individual change in a component.
...
!\[\[Brookjs components strawman proposal#^TodoList-js]]
...
!\[\[Brookjs components strawman proposal#^TodoListWithIterationHelper-js]]
...
!\[\[Brookjs components strawman proposal#^client-js]]
...
!\[\[Brookjs components strawman proposal#^client-with-domDelta-js]]
